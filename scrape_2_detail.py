...
from scrape_1_index import db, get_or_retry
...
def get_detail_page(recipe_id):
    ...
    with db:
        res = db.execute(
            "update recipes set detail_html = :html where id = :id and detail_html is null",
            {"html": html, "id": recipe_id},
        )
        if res.rowcount != 1:
...
if __name__ == "__main__":
    missing_recipes = db.execute(
        "select * from recipes where detail_html is null"
    ).fetchall()
